...
public class DBOpenHelper {
    private static final String DATABASE_NAME = "readywomen.db";
     ...
     */
    public long insertColumn(String name, String term, String time, String price, String status, String center) {
        ContentValues values = new ContentValues();
        values.put(Database.CreateDB.CLASSNAME, name);
        values.put(Database.CreateDB.CLASSTERM, term);
        values.put(Database.CreateDB.CLASSTIME, time);
        values.put(Database.CreateDB.CLASSPRICE, price);
        values.put(Database.CreateDB.CLASSSTATUS, status);
        values.put(Database.CreateDB.CLASSCENTER, center);
     ...
     */
    public boolean updateColumn(long id, String name, String term, String time, String price, String center, String status) {
        ContentValues values = new ContentValues();
        values.put(Database.CreateDB.CLASSNAME, name);
        values.put(Database.CreateDB.CLASSTERM, term);
        values.put(Database.CreateDB.CLASSTIME, time);
    ...
    //입력한 프로그램이름을 가진 DB를 지우는 메소드
    public boolean deleteColumn(String name){
        return mDB.delete(Database.CreateDB._TABLENAME, "name="+name, null)>0;
    }

    //이름으로 검색하기
    public Cursor getMatchName(String name){
        Cursor c = mDB.rawQuery("select * from programTable where name"+""+name+"", null);
        return c;
    }
